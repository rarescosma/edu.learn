object tw {
  ...
  };import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(2792); 
  
  val s1 = new NonEmpty(new Tweet(1), new Empty, new Empty);System.out.println("""s1  : tw.NonEmpty = """ + $show(s1 ));$skip(275); 
  ...
  val l1 = List(new Tweet(1), new Tweet(5));System.out.println("""l1  : List[tw.Tweet] = """ + $show(l1 ));$skip(58); 
  val l2 = List(new Tweet(2), new Tweet(4), new Tweet(6));System.out.println("""l2  : List[tw.Tweet] = """ + $show(l2 ));$skip(125); 
  val mergeTweetLists: (List[Tweet], List[Tweet]) => List[Tweet] =
    merge[Tweet]((t1, t2) => (t1.retweets < t2.retweets));System.out.println("""mergeTweetLists  : (List[tw.Tweet], List[tw.Tweet]) => List[tw.Tweet] = """ + $show(mergeTweetLists ))}
}
